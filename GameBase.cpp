...
bool GameBase::loadGameFiles() {  // *Developed with AI assistance*
    ...
    if (foundFiles.empty())
    {
        handleError("No game files found.");
        return false;
    }
...
bool GameBase::loadRiddles() {
    ...
    if (!file.is_open()) {
        handleError("Cannot load riddles file");
        return false;
    }
    ...
    while (file >> roomID >> x >> y) {
        ...
        if (!r) {
            handleError("Rule refers to non-existing riddle in room " + std::to_string(roomID) +
...
void GameBase::moveRoom(Player& player, int dest) {
    ...
    onScreenChange(static_cast<PlayerID>(idx), dest);
...
bool GameBase::handleRiddles(Player& player, bool solved) {
    Screen& room = screens[currRoomID];
    const Point& nextPos = player.getNextPos();
    ...
    PlayerID id = getPlayerID(player);
    onRiddle(id, solved);
...
void GameBase::processKey(char ch) {
    ...
    for (PlayerID id: {PLAYER_1, PLAYER_2}) {
        ...
        if (players[id].isDisposeKey(c)) {
            handleDispose(players[id]);
            return;
        }

        // Movement keys
        if (players[id].isMoveKey(c)){
            players[id].setDir(c);
            return;
        }
    }
